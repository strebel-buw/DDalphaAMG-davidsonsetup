...
void vector_PRECISION_projection( vector_PRECISION z, vector_PRECISION v, int k, vector_PRECISION *W, int orthogonal, 
                                  level_struct *l, Thread *threading ) {
  
  int start, end;
  complex_PRECISION ip[k], ip_buffer[2*k];

  if (orthogonal) ASSERT(z != v);
  compute_core_start_end( 0, l->inner_vector_size, &start, &end, l, threading );
  
  process_multi_inner_product_PRECISION( k, ip, W, v, start, end, l, threading );
  ...
  END_MASTER(threading)
  SYNC_MASTER_TO_ALL(threading)
  vector_PRECISION_define( z, 0, start, end, l );
  vector_PRECISION_multi_saxpy( z, W, ip_buffer+k, 1, k, start, end, l );
  
  if (orthogonal) vector_PRECISION_minus( z, v, z, start, end, l );
}
