...
void next_level_setup( vector_double *V, level_struct *l, struct Thread *threading ) {
  ...
  if ( l->level > 0 ) {
    ...
    if ( g.mixed_precision ) {
      ...
      if ( l->depth == 0 ) {
        START_LOCKED_MASTER(threading)
        if( g.davidson_setup ) davidson_setup_float_struct_alloc( 1, 5*l->num_eig_vect, 1E-8, &(l->d_float), l );
        interpolation_float_alloc( l );
        END_LOCKED_MASTER(threading)
        if( g.davidson_setup ) initial_davidson_setup_float( 1, &(l->d_float), l, threading );
        else interpolation_float_define( V, l, threading );
      ...
      if ( l->depth == 0 ) {
        START_LOCKED_MASTER(threading)
        if( g.davidson_setup ) davidson_setup_double_struct_alloc( 1, 5*l->num_eig_vect, 1E-4, &(l->d_double), l );
        interpolation_double_alloc( l );
        END_LOCKED_MASTER(threading)
        if( g.davidson_setup ) initial_davidson_setup_double( 1, &(l->d_double), l, threading );
        else interpolation_double_define( V, l, threading );
...
void read_solver_parameters( FILE *in, level_struct *l ) {
  ...
  if ( g.num_levels == 1 ) g.interpolation = 0; 
  else {
    save_pt = &(g.interpolation); g.interpolation = 2;
    read_parameter( &save_pt, "interpolation:", "%d", 1, in, _DEFAULT_SET );
  }
  save_pt = &(g.davidson_setup); g.davidson_setup = 0;
  read_parameter( &save_pt, "davidson setup:", "%d", 1, in, _DEFAULT_SET );
