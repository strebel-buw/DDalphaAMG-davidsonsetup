...
void coarse_grid_correction_PRECISION_setup( level_struct *l, struct Thread *threading ) {
  ...
  if ( l->next_level->level > 0 ) {
    ...
    if ( !l->next_level->idle ) {
      if( g.davidson_setup )  initial_davidson_setup_PRECISION( 1, &(l->next_level->d_PRECISION), l->next_level, threading );
      else interpolation_PRECISION_define( NULL, l->next_level, threading );
    }
  ...
  if ( !l->idle ) {
    if ( !l->next_level->idle ) {
      if ( l->next_level->level > 0 ) {
        ...
        if( g.davidson_setup ) davidson_setup_PRECISION_struct_free( &(l->next_level->d_PRECISION), l->next_level );
    ...
    if( g.davidson_setup ) davidson_setup_PRECISION_struct_free( &(l->d_PRECISION), l );
...
//         vector_PRECISION_define_random( buffer[0], 0, l->inner_vector_size, l );
        vector_PRECISION_define( buffer[0], 0, 0, l->inner_vector_size, l );
        buffer[0][k] = 1;
        END_LOCKED_MASTER(threading)
      }
  
      smoother_PRECISION( l->is_PRECISION.test_vector[k], NULL, buffer[0], 4, _NO_RES, _NO_SHIFT, l, threading );
//       smoother_PRECISION( buffer[0], NULL, l->is_PRECISION.test_vector[k], g.method>=4?1:2, _NO_RES, _NO_SHIFT, l, threading );
//       smoother_PRECISION( l->is_PRECISION.test_vector[k], NULL, buffer[0], g.method>=4?1:3, _NO_RES, _NO_SHIFT, l, threading );
    ...
    for ( k=0; k<n; k++ ) {
      vector_PRECISION_real_scale( l->is_PRECISION.test_vector[k], l->is_PRECISION.test_vector[k],
                                  ...
                                  start, end, l );
//       PRECISION tmp = global_norm_PRECISION( l->is_PRECISION.test_vector[k], 0, l->inner_vector_size, l, threading );
//       printf0("\n|TV[%d]|=%lf\n", k, tmp);
//       for( int j = 0; j<5; j++ ) {
//         printf0("%+.8lf%+.8lfi ", CSPLIT(l->is_PRECISION.test_vector[k][j]));
//       }
    }
...
//   for( int k = 0; k<5; k++ ) {
//     PRECISION tmp = global_norm_PRECISION( l->is_PRECISION.interpolation[k], 0, l->inner_vector_size, l, threading );
//     printf0("\n|TV[%d]|=%lf\n", k, tmp);
//     for( int j = 0; j<5; j++ ) {
//       printf0("%+.8lf%+.8lfi ", CSPLIT(l->is_PRECISION.interpolation[k][j]));
//     }
//   }
